...
### Fixed
- **"Take photo" no longer blames the camera when push is what is missing (#524).** On an install without FCM credentials the button waited out a 15-second timeout and then said the camera might be offline. The camera was fine: the captured photo comes back over the push channel, so with no credentials there was never going to be a photo. The press now fails immediately with the message that was already written for this, and it fails *before* asking the hub — the guard that was supposed to do this had been checking whether a listener object existed, which is always true because setup starts the listener credentials or not. **Requests to Ajax:** one fewer — a capture that could not produce a photo is no longer sent.
...
Housekeeping, not features: the diagnostics dump starts recording what the hub's
battery updates carry, and the integration finally declares the Home Assistant
version it can actually be installed on.

Confirmed on a live install (Home Assistant 2026.9.2) as `1.21.0-beta.1`: the new
diagnostics block is present and the integration ran clean with no errors. The
counts it collects are, by design, only meaningful once installs have been running
for a while — waiting for them before releasing would have delayed the very data
they exist to gather.

**Requests to Ajax: none added.** Both changes ride connections that are already
open.
...
### Changed
- **The declared minimum Home Assistant version is now `2025.11.0`, which is the oldest release the integration can actually be installed on (#513).** It previously advertised `2024.1.0`, a version it has never been able to run on. The reason is not the Home Assistant APIs the code calls: Home Assistant installs an integration's requirements through pip using its own constraints file, and that file pins `grpcio` to one exact version. This integration needs `grpcio` 1.75.1 or newer — the version its generated protocol code was compiled against — and Home Assistant pinned an older one up to and including `2025.10`. On those releases the install cannot resolve at all and setup ends in *"Requirements for aegis_ajax not found"*, with nothing in the integration's own logs to explain it. Anyone already running it is on a release well past that line and is unaffected; anyone below it was never able to install it and now gets told so by HACS, up front, instead of by a setup failure. The test matrix moved with it, to Python 3.13 and 3.14, so the integration is now tested against the Home Assistant release most installations are actually running rather than one six releases behind. **Requests to Ajax:** none added.
...
ring stops landing on another client's entity.

None of these changes adds a periodic request to Ajax. The alarm-image import runs on a real
camera alarm and opens exactly one media stream, the same one the Ajax app opens when the user
taps that notification; the manual backfill runs only when called and refuses to run more than
once every five minutes per space. The Outdoor 2/4 PhOD change reclassifies a device family, so
its only requests are the ones a button press or a real alarm already makes on the MotionCams
...
- **A push registration that is accepted and never delivers is now detected (#437).** The four FCM values can pass the offline shape checks, mint a token at Firebase, be accepted by Ajax's own registration call, and hold a connection that heartbeats indefinitely, without one message ever arriving — and nothing noticed, because the alarm panel never depends on push. The only symptom was that doorbell rings and intrusion alarms silently never happened, which nobody has a baseline for; #359 sat in exactly that state for a month. The hub's status stream carries the same space events push does, about a second apart on a healthy install, so arm/disarm-class events seen while the push client was up are counted as demonstrated opportunities. Twenty of them with no push ever delivered for the credentials in force raises a Repair card. The count is persisted against a fingerprint of the four values, so it survives restarts — the reason a month of silence never accumulated into anything was that the old timestamp reset on every one — and re-entering the credentials resets the evidence by construction. A quiet installation never trips it, since no events means no evidence either way. The card names the three causes and deliberately does not choose between them, because from inside Home Assistant they are indistinguishable: values belonging to a Firebase project the co-branded app's push provider does not route through, another registration holding the account's push destination, or a connection that never completed. The threshold is set well past the point of doubt on purpose, because the cost of a false positive is telling someone whose setup works that it does not. Repair text translated into six languages; the rest carry the English wording and are tracked on #475. No additional requests to Ajax: every value it reads is already flowing.

### Fixed
- **The MotionCam Outdoor 2/4 PhOD gets its camera entity and capture button (#499).** The list of families that receive photo entities predates #408, which took the modelled families from 33 to 107, and was never revisited: `motion_cam_outdoor_two_four_phod` was fully modelled as a detector, with its motion, tamper and delay sensors, but received neither the camera entity that shows the last capture nor the button that requests a new one. It is now in the photo-on-demand group, and the capture request announces it as an outdoor MotionCam. That second half matters as much as the first: the wire request carries a coarse device type of its own, with only indoor, outdoor and Fibra-base values, and a family missing from that table silently fell back to indoor. Exposing the button without widening the table produced a button that always failed, which is what `1.20.0-beta.3` shipped and @Cris59264 caught within the hour. The two lists are now pinned together by a test, so the handler registry cannot outrun the wire table again, and an unmapped family logs a warning instead of quietly claiming to be indoor. The family was moved on field evidence rather than on the `phod` in its name, which is the bar this deliberately holds: @Cris59264 reported an install where the Ajax app itself offers Photo on Demand for the device, and the attached diagnostics carried two of them alongside a working `motion_cam_phod` on the same hub — the same account, the same permissions and the same integration version, with one family getting the entities and the other not. The remaining unmapped MotionCam families are unchanged and still waiting on the same kind of confirmation, tracked on #472. As with every Photo on Demand device, a capture spends the detector's own battery, so the button is a deliberate action and not something to poll. **Requests to Ajax:** no periodic request is added. The capture button sends one request per press, the same one the app sends when the user taps *Take a photo*. The alarm-image import now also accepts alarms from this family, which opens one media stream per real camera alarm exactly as it already does for the other MotionCams; the manual backfill passes the widened device set inside its existing single history query per space, so its request count is unchanged.
...
- **The diagnostics dump records whether a degraded snapshot has ever held a deactivation (#419).** The carry that keeps a deactivated device from silently reading as protecting corrects itself within one status refresh, so its only trace was a debug line — and the reporter watching for a natural occurrence lost the decisive moment to log rotation twice, which is what has kept the fix unconfirmed rather than any doubt about the code. The dump now carries how many snapshots have held a deactivation, how many devices that covered, when the last one was, which devices a carry currently applies to, and the age of the hub's bypass report per device, which is the first thing to look at when the count stays at zero. Counts only ever grow, so they still show that a carry happened after the hub's next report withdrew it. Device ids only, no names. No additional requests to Ajax.
