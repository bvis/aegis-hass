...
### Fixed
- **Siren volume and alarm duration now confirm within seconds after you change them.** Previously the number/select value could keep showing the *previous* setting for up to ~15 minutes after a successful change (the entities have no optimistic state and only refreshed on the shared background snapshot timer), which from the UI was indistinguishable from a rejected write. A change now triggers an immediate authoritative read-back of that siren's settings, so the confirmed value appears within seconds. The read-back is deliberately a real snapshot read rather than an optimistic guess, so a hub that accepts but doesn't apply a change is surfaced instead of hidden. Thanks to @wip3out3r for the hardware validation that surfaced this.
