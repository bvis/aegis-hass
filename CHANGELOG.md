...
### Fixed
- **Alarm panel now follows an app-side arm/disarm instantly over HTS (#258).** Arming or disarming from the Ajax app emits a hub event that shares the same signature as the Chime-toggle event, so it was mis-routed to the chime handler and the panel only caught up on the next poll (up to a few minutes later). The event's state byte is now decoded directly — disarm / arm away / night mode — and applied immediately, the same way Chime already was. Partial/group arm modes aren't decoded from the event yet and still reconcile on the periodic poll.
